...
async function carregarDados() {
  try {
    const resp = await fetch("data/infra_index.json?v=" + Date.now());
...
function renderizarTabela(lista) {
  ...
  const colunas = ["Ticker","Tipo","Preço Atual","Variação Dia","P/VP","DY a.a.","Retorno - MTD","Retorno - 12M","Último Dividendo Pago"];

  lista.forEach(f => {
    const tr = document.createElement("tr");
    tr.onclick = () => { window.location.href = "infra_fundo.html?ticker=" + encodeURIComponent(f["Ticker"]); };
